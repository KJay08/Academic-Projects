...
$(document).ready(function ()
{
    ...
    {
        // Top row
        $(`section[suite="01${i.toString().padStart(2, '0')}"]`).css({
            "grid-column": i,
            "grid-row": 1
        });

        // Bottom row
        $(`section[suite="11${i.toString().padStart(2, '0')}"]`).css({
            "grid-column": i,
            "grid-row": 11
        });
    }
    ...
    {
        // Left column
        $(`section[suite="${i.toString().padStart(2, '0')}01"]`).css({
            "grid-column": 1,
            "grid-row": i
        });

        // Right column
        $(`section[suite="${i.toString().padStart(2, '0')}11"]`).css({
            "grid-column": 11,
            "grid-row": i
        });
    }

    $('[suite = "0102"]').addClass("red bottom");
    $('[suite = "0104"]').addClass("red bottom");
    $('[suite = "0105"]').addClass("red bottom");
    $('[suite = "0107"]').addClass("yellow bottom");
    $('[suite = "0108"]').addClass("yellow bottom");
    $('[suite = "0110"]').addClass("yellow bottom");
    $('[suite = "0311"]').addClass("green left");
    $('[suite = "0411"]').addClass("green left");
    $('[suite = "0611"]').addClass("green left");
    $('[suite = "0911"]').addClass("blue left");
    $('[suite = "1011"]').addClass("blue left");
    $('[suite = "1110"]').addClass("brown top");
    $('[suite = "1108"]').addClass("brown top");
    $('[suite = "1105"]').addClass("lightblue top");
    $('[suite = "1103"]').addClass("lightblue top");
    $('[suite = "1102"]').addClass("lightblue top");
    $('[suite = "1001"]').addClass("purple right");
    $('[suite = "0801"]').addClass("purple right");
    $('[suite = "0701"]').addClass("purple right");
    $('[suite = "0501"]').addClass("orange right");
    $('[suite = "0301"]').addClass("orange right");
    $('[suite = "0201"]').addClass("orange right");
    ...
    $("#loadPrices").click(() =>
    {
        console.log("Inside button loadPrices");

        let data = {
            action : "propertyPrices"
        }

        MakeAjaxCall(URL, "POST", "JSON", data,
            function (result)
            {
                console.log(result);
                ...
                // Loop through all 40 board squares
                $('section').toArray().forEach((el, index) =>
                {
                    const $el = $(el);
                    ...
                    if (value !== undefined)
                    {
                        ...
                        const text = $.trim($el.text().split('$')[0]); // Remove old $ value
                        if (value > 0)
                        {
                            $el.html(`${text} <br> $${value}`);
                        } else
                        {
                            $el.html(text); // No price for -1
                        }
                    }
                });

                $("#RollDice").prop('disabled', false);
                $("#loadPrices").prop('disabled', true);
            },
            function (xhr, status, error)
            {
                console.error("Error loading property prices:", status, error);
                alert("Failed to load property prices.");
            })
    })
    ...
    function MakeAjaxCall(url, type, dataType, data, successHandler, errorHandler)
    {
    ...
    function rollDiceAndMove()
    {
        ...
        $.ajax({
            ...
            {
                console.log("Dice Roll:", response);

                const die1 = parseInt(response.dice1);
                const die2 = parseInt(response.dice2);
            ...
            {
                console.error("Dice roll failed", status, error);
                alert("Error rolling dice. Try again.");
            }
        })
    }
